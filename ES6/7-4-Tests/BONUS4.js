...
function convertToRoman(numero) {
  ...
  if (digits.length === 1) return convertDigit(Number(digits[0]), 'I', 'V');
    ...
    return `${convertDigit(Number(digits[0]), 'X', 'L')}${convertDigit(
      Number(digits[1]),
      'I',
      'V'
    )}`;
  else if (digits.length === 3)
    return `${convertDigit(Number(digits[0]), 'C', 'D')}${convertDigit(
      Number(digits[1]),
      'X',
      'L'
    )}${convertDigit(Number(digits[2]), 'I', 'V')}`;
  else if (digits.length === 4)
    return `${'M'.repeat(Number(digits[0]))}${convertDigit(
      Number(digits[1]),
      'C',
      'D'
    )}${convertDigit(Number(digits[2]), 'X', 'L')}${convertDigit(
      Number(digits[3]),
      'I',
      'V'
    )}`;
